...
var embededLnd []byte
var LndExePath string

var (
	LndConfigPath string
	LndConfigFile string
)
...
func (l LndService) fmtLog(lt LogType, desc string) *Log {
	log := &Log{
		date:    time.Now(),
		desc:    desc,
		logType: lt,
...
func (ts LndService) start(ctx context.Context, onReady func(), onStop func(*Log), onLog func(*Log)) {
	if onReady == nil || onStop == nil || onLog == nil {
		panic("Some function parameter to lnd start method is missing.")
	}
	ts.onReady = onReady
	ts.onStop = onStop
	ts.onLog = onLog
	...
	LndExePath = filepath.Join(ServiceRootDir, "embed", "lnd", "lnd")
	if err := InstallExe(embededLnd, LndExePath, onLog); err != nil && err.Error() != "installed" {
		log := ts.fmtLog(FATAL, err.Error())
		go onLog(log)
		go onStop(log)
		return
	} else if err != nil && err.Error() == "installed" {
		go onLog(ts.fmtLog(INFO, "lnd executable installed under "+LndExePath))
	}

	LndConfigPath = filepath.Join(ServiceRootDir, "lnd")
	err := os.MkdirAll(LndConfigPath, 0755) // make sure the log directory exists
	if err != nil {
		log := ts.fmtLog(FATAL, fmt.Sprintf("Cannot create lnd directory %v: ", err))
		go onLog(log)
		go onStop(log)
		return
	}
...
func (ts LndService) onLogHook() func(*Log) {
	return ts.onLog
}

func (ts LndService) onReadyHook() func() {
	return ts.onReady
}

func (ts LndService) getConfigFile() (string, error) {
	return LndConfigFile, nil // assuming that the torrc file path is stored as a global variable LndConfigPath
}

func (ts LndService) name() string {
	return "Lnd"
}
